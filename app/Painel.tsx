...
export default function Painel({ initialIsAdmin, userSession }: any) {
  ...
  const muralMês = registros.filter(r => 
    (r.status === 'APROVADO' || r.status === 'ARQUIVADO') && 
    r.criado_em.startsWith(mesAtualStr) &&
                ...
                {equipeFiltrada.map(m => (
                  <div key={m.discordId} onClick={() => setMembroSelecionado(m)} className="bg-[#0a0a0a] p-8 rounded-[2.5rem] border border-white/5 hover:border-yellow-400/50 transition-all cursor-pointer group shadow-xl">
                    ...
                    <div className="pt-6 border-t border-white/5 space-y-3 text-[10px] font-black uppercase tracking-widest">
                       <div className="flex justify-between text-zinc-500"><span>BRUTO:</span><span className="text-white font-mono">R$ {formatMoney(m.bruto)}</span></div>
                       <div className="flex justify-between text-green-500/80"><span>LÍQUIDO:</span><span className="text-green-400 font-mono">R$ {formatMoney(m.liq)}</span></div>
             ...
             <div className="bg-[#0a0a0a] border border-white/5 rounded-[3.5rem] p-12 shadow-2xl">
                <form onSubmit={handleEnviar} className="space-y-6">
                   <div className="space-y-2 border-b border-white/5 pb-6 mb-6">
                        <label className="text-[10px] uppercase tracking-widest text-yellow-400 ml-2 font-black">MEMBRO RESPONSÁVEL</label>
                        <select value={form.vendedorId || form.membroSaqueId} onChange={e => setForm({...form, vendedorId: e.target.value, membroSaqueId: e.target.value})} className="w-full bg-black border border-white/10 p-6 rounded-2xl text-white outline-none focus:border-yellow-400 font-black uppercase text-xs appearance-none cursor-pointer" required>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 animate-in fade-in">
             ...
             {pendencias.map(r => (
               <div key={r.id} className="bg-[#0a0a0a] p-10 rounded-[3rem] border border-red-500/20 shadow-2xl flex flex-col justify-between group">
                  ...
                  <button onClick={() => setModalParcela(r)} className="w-full bg-green-500 text-black font-black py-5 rounded-2xl uppercase text-[10px] hover:bg-green-400">RECEBER PARCELA</button>
               </div>
             ))}
          </div>
        )}
             ...
             {aguardando.map(r => (
               <div key={r.id} className="bg-[#0a0a0a] p-10 rounded-[3rem] border border-yellow-400/20 shadow-2xl flex flex-col justify-between">
                  ...
                  <div className="flex gap-3">
                    <button onClick={() => decidir(r.id, 'APROVAR')} className="flex-1 bg-yellow-400 text-black font-black py-4 rounded-xl uppercase text-[10px] hover:bg-yellow-300">APROVAR</button>
        ...
        {activeTab === 'historico_backup' && (
          <div className="space-y-8 animate-in fade-in">
             ...
             <div className="bg-[#0a0a0a] border border-white/5 rounded-[3.5rem] overflow-hidden shadow-2xl">
                 <table className="w-full text-left font-black uppercase text-[10px] tracking-widest whitespace-nowrap min-w-max">
                   <thead className="bg-white/5 text-zinc-500 border-b border-white/5">
                     <tr><th className="px-8 py-6">MEMBRO</th><th className="px-8 py-6">TIPO</th><th className="px-8 py-6">VALOR</th><th className="px-8 py-6">DATA</th></tr>
                   </thead>
                   <tbody className="divide-y divide-white/5">
                 ...
                 <div className="space-y-6">
                    <div className="grid grid-cols-2 gap-6">
                       <div className="bg-[#0a0a0a] border border-white/5 p-8 rounded-[2.5rem]">
                          <p className="text-[10px] text-zinc-600 font-black uppercase mb-3">Produção (Mês)</p>
                          <p className="text-3xl font-mono italic text-white font-black">R$ {formatMoney(membroSelecionado.bruto)}</p>
                       </div>
                       <div className="bg-[#0a0a0a] border border-white/5 p-8 rounded-[2.5rem]">
                          <p className="text-[10px] text-zinc-600 font-black uppercase mb-3">Líquido (Caixa)</p>
                          <p className="text-3xl font-mono italic text-green-400 font-black">R$ {formatMoney(membroSelecionado.liq)}</p>
                       </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-6">
                      <div className="bg-blue-500/5 border border-blue-500/10 p-6 rounded-[2rem]"><p className="text-[9px] font-black text-blue-500 mb-1 uppercase">Corridinhas</p><p className="text-2xl font-mono text-blue-400">+R$ {formatMoney(membroSelecionado.corridinhas)}</p></div>
                      <div className="bg-red-500/5 border border-red-500/10 p-6 rounded-[2rem]"><p className="text-[9px] font-black text-red-500 mb-1 uppercase">Já Pago</p><p className="text-2xl font-mono text-red-400">-R$ {formatMoney(membroSelecionado.pago)}</p></div>
                    </div>
                 </div>
                    ...
                    {registros.filter(r => String(r.discordId) === String(membroSelecionado.discordId) && (r.status === 'APROVADO' || r.status === 'ARQUIVADO') && r.criado_em.startsWith(mesAtualStr) && !(r.item || '').toUpperCase().includes('SALDO RETIDO')).map((r: any) => (
                      <div key={r.id} className="flex justify-between items-center bg-[#0a0a0a] p-6 rounded-[2rem] border border-white/5 hover:border-white/10 transition-colors">
                         <div>
                            ...
                            <p className="text-[9px] text-zinc-600 tracking-widest uppercase mt-1.5">{new Date(r.criado_em).toLocaleDateString('pt-BR')} • {r.cliente || 'SISTEMA'}</p>
                         </div>
