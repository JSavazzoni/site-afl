...
export default function Painel({ initialIsAdmin, userSession }: any) {
  ...
  // --- MATEMÁTICA FINANCEIRA (SOMA CORRIDINHAS NO SALDO) ---
  ...
  // --- PROCESSAMENTO DE DADOS (FILTROS DE EXIBIÇÃO) ---
  ...
  // Lógica de Backup (Resolvendo image_8fb943)
  ...
  // --- FUNÇÕES DE COMANDO (Resolvendo image_8fb960) ---
  ...
  const virarMes = async () => {
    if (!confirm("AVISO: Isso irá ARQUIVAR Março e zerar as produções. Continuar?")) return;
    setLoading(true);
  ...
  return (
    <div className="flex min-h-screen bg-[#050505] text-white font-sans selection:bg-yellow-400 overflow-hidden">
        ...
        <nav className="flex-1 space-y-2">
          ...
          {isAdmin && (
            <div className="pt-6 mt-6 border-t border-white/5 space-y-2">
              ...
              <button onClick={() => setActiveTab('pendencias')} className={`w-full flex items-center justify-between px-6 py-4 rounded-2xl transition-all ${activeTab === 'pendencias' ? 'bg-yellow-400/10 text-yellow-400 border border-yellow-400/20 shadow-lg' : 'text-zinc-500 hover:text-white hover:bg-white/5'}`}>
      ...
      <main className="flex-1 p-12 overflow-y-auto bg-[#050505]">
        <header className="mb-16 flex justify-between items-end border-b border-white/5 pb-8">
          ...
          <div className="flex items-center gap-4 bg-[#0a0a0a] p-4 rounded-[2rem] border border-white/5">
             <div className="pr-2 text-right">
                ...
                <p className="text-[9px] font-bold text-yellow-400 uppercase tracking-widest mt-1">{isAdmin ? 'ADMINISTRADOR' : 'AGENTE AFL'}</p>
             </div>
                ...
                {equipeFiltrada.map(m => (
                  <div key={m.discordId} onClick={() => setMembroSelecionado(m)} className="bg-[#0a0a0a] p-8 rounded-[2.5rem] border border-white/5 hover:border-yellow-400/50 cursor-pointer shadow-xl group transition-all">
                    ...
                    <div className="pt-6 border-t border-white/5 space-y-2 text-[10px] font-black uppercase">
                       ...
                       <div className="flex justify-between text-yellow-400 bg-yellow-400/5 p-2 rounded-lg mt-2 font-mono"><span>SALDO:</span><span>R$ {formatMoney(calcAReceber(m))}</span></div>
                    </div>
                  </div>
                ))}
             </div>
          ...
          <div className="max-w-2xl mx-auto animate-in zoom-in-95">
             ...
             <form onSubmit={handleEnviar} className="bg-[#0a0a0a] border border-white/5 rounded-[3.5rem] p-12 shadow-2xl space-y-6">
                <div className="space-y-2 border-b border-white/5 pb-6 mb-6">
                  ...
                  <select value={form.vendedorId || form.membroSaqueId} onChange={e => setForm({...form, vendedorId: e.target.value, membroSaqueId: e.target.value})} className="w-full bg-black border border-white/10 p-6 rounded-2xl text-white outline-none focus:border-yellow-400 font-black uppercase text-xs appearance-none" required>
                ...
                {form.tipo === 'VENDA' && (
                  <div className="space-y-6">
                    <InputField label="CLIENTE" value={form.cliente} onChange={(v:any)=>setForm({...form, cliente: v})} placeholder="Ex: Lucas | 4116" />
                    <InputField label="ITEM" value={form.item} onChange={(v:any)=>setForm({...form, item: v})} placeholder="Ex: Farm de Dinheiro" />
                ...
                {form.tipo === 'SAQUE' && <InputField label="VALOR TRANSFERIDO (R$)" type="number" value={form.valor} onChange={(v:any)=>setForm({...form, valor: v})} placeholder="Dinheiro pago ao membro" />}
                <button disabled={loading} className="w-full bg-yellow-400 text-black font-black py-6 rounded-2xl uppercase tracking-[0.3em] text-xs shadow-xl hover:bg-yellow-300">
        ...
        {activeTab === 'pendencias' && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 animate-in fade-in">
             {pendencias.length === 0 && <p className="col-span-full text-zinc-600 font-black uppercase text-center py-20">Nenhuma dívida encontrada.</p>}
             {pendencias.map(r => (
               <div key={r.id} className="bg-[#0a0a0a] p-10 rounded-[3rem] border border-red-500/20 shadow-2xl flex flex-col justify-between group relative overflow-hidden">
                  ...
                  <button onClick={() => setModalParcela(r)} className="w-full bg-green-500 text-black font-black py-5 rounded-2xl uppercase text-[10px] hover:bg-green-400 shadow-lg relative z-10">RECEBER PARCELA</button>
               </div>
             ))}
          </div>
        )}
        ...
        {activeTab === 'admin_zone' && (
           <div className="space-y-8 animate-in fade-in">
             <div className="bg-red-500/5 border border-red-500/20 p-10 rounded-[3rem] max-w-xl shadow-lg">
                <h3 className="text-3xl font-black italic text-white mb-4">FECHAMENTO MENSAL</h3>
                <p className="text-zinc-500 mb-8 text-[10px] font-black uppercase">Arquiva o mês e salva os saldos. USE APENAS NO DIA 1º.</p>
                   ...
                   <tbody className="divide-y divide-white/5">
                     {registros.filter(r => r.status === 'APROVADO').map((r: any) => (
                       <tr key={r.id} className="hover:bg-white/[0.02] transition-colors">
                         ...
                         <td className="px-8 py-6 text-right"><button onClick={() => deletarLog(r.id)} className="text-red-500/50 hover:text-red-500"><Trash2 size={16}/></button></td>
                       </tr>
                     ))}
                   </tbody>
                 </table>
          ...
          <div className="space-y-8 animate-in fade-in">
             <div className="flex flex-col md:flex-row md:items-center gap-6 bg-[#0a0a0a] p-8 rounded-[3rem] border border-white/5">
                 <div className="flex-1">
                    <p className="text-[10px] font-black uppercase text-zinc-500 mb-2">MÊS DE REFERÊNCIA</p>
                 ...
                 <div className="flex-1 flex gap-6">
                    <div className="flex-1 bg-black p-4 rounded-2xl border border-white/5">
                        <p className="text-[9px] text-zinc-600 font-black mb-1 uppercase tracking-widest">BRUTO</p>
                    ...
                    <div className="flex-1 bg-black p-4 rounded-2xl border border-green-500/20">
                        <p className="text-[9px] text-green-500/70 font-black mb-1 uppercase tracking-widest">CAIXA</p>
                        <p className="text-xl font-mono text-green-400 italic font-black">R$ {formatMoney(backupLiquido)}</p>
                    </div>
                 </div>
             </div>
             <div className="bg-[#0a0a0a] border border-white/5 rounded-[3.5rem] overflow-hidden shadow-2xl">
                 <table className="w-full text-left font-black uppercase text-[10px] tracking-widest min-w-max">
                   <thead className="bg-white/5 text-zinc-500 border-b border-white/5">
           ...
           <div className="bg-[#050505] border border-white/10 w-full max-w-5xl rounded-[4rem] flex flex-col max-h-[90vh] overflow-hidden shadow-2xl">
              ...
              <div className="p-12 overflow-y-auto flex-1 grid grid-cols-1 lg:grid-cols-2 gap-12">
                 <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-6">
                       <div className="bg-[#0a0a0a] p-6 rounded-[2rem] border border-white/5"><p className="text-[9px] text-zinc-600 font-black uppercase mb-1">Bruto (Mês)</p><p className="text-xl font-mono text-white">R$ {formatMoney(membroSelecionado.bruto)}</p></div>
                       <div className="bg-[#0a0a0a] p-6 rounded-[2rem] border border-white/5"><p className="text-[9px] text-zinc-600 font-black uppercase mb-1">Caixa (Mês)</p><p className="text-xl font-mono text-white">R$ {formatMoney(membroSelecionado.liq)}</p></div>
                    </div>
                 </div>
                 <div className="space-y-4">
                    ...
                    {registros.filter(r => String(r.discordId) === String(membroSelecionado.discordId) && (r.status === 'APROVADO' || r.status === 'ARQUIVADO') && r.criado_em.startsWith(mesAtualStr) && !(r.item || '').toUpperCase().includes('SALDO RETIDO')).map((r: any) => (
                      <div key={r.id} className="flex justify-between items-center bg-[#0a0a0a] p-6 rounded-[2rem] border border-white/5 hover:border-white/10 transition-colors">
                         <div>
                            <p className={`font-black text-sm uppercase italic ${r.tipo === 'CORRIDINHA' ? 'text-blue-400' : 'text-white'}`}>{r.item || r.tipo}</p>
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/95 p-4 animate-in zoom-in-95">
           <div className="bg-[#0a0a0a] border border-zinc-800 p-12 rounded-[3.5rem] w-full max-w-md shadow-2xl relative">
              ...
              <form onSubmit={handlePagarParcela} className="space-y-8">
                 <div className="bg-black border border-white/5 p-6 rounded-3xl text-center mb-4"><p className="text-[10px] text-zinc-500 font-black uppercase mb-1">CLIENTE</p><p className="text-xl font-black text-white italic truncate">{modalParcela.cliente}</p></div>
                 <InputField label={`VALOR RECEBIDO (FALTAM R$ ${formatMoney(Number(modalParcela.valor) - Number(modalParcela.valorRecebido))})`} type="number" value={valorParcela} onChange={setValorParcela} placeholder="R$ 0,00" />
                 ...
                 <button disabled={loading} className="w-full bg-green-500 text-black font-black py-6 rounded-2xl uppercase tracking-widest text-xs hover:bg-green-400 transition-all shadow-lg mt-4">
      ...
      {/* FOOTER DO ZK */}
      <footer className="fixed bottom-0 right-0 p-8 pointer-events-none opacity-20">
         <p className="text-[8px] font-black uppercase tracking-[1em] text-white">Coded by Zk</p>
      </footer>
