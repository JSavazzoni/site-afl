...
export default function Painel({ initialIsAdmin, userSession }: any) {
  ...
  const virarMes = async () => {
    if (!confirm("AVISO: Isso irá arquivar o mês atual. Deseja prosseguir?")) return;
    setLoading(true);
  ...
  const forcarAuditoria = async () => {
    if (!confirm("Isso irá RESTAURAR todas as vendas de Março. Continuar?")) return;
    setLoading(true);
  ...
  return (
    <div className="flex min-h-screen bg-[#050505] text-white font-sans selection:bg-yellow-400 overflow-hidden">
      ...
      <aside className="w-64 border-r border-white/5 bg-[#0a0a0a] p-6 flex flex-col z-50">
        ...
        <nav className="flex-1 space-y-2">
          ...
          {isAdmin && (
            <div className="pt-6 mt-6 border-t border-white/5 space-y-2">
              ...
              <button onClick={() => setActiveTab('pendencias')} className={`w-full flex items-center justify-between px-6 py-4 rounded-2xl transition-all ${activeTab === 'pendencias' ? 'bg-yellow-400/10 text-yellow-400 border border-yellow-400/20 shadow-lg' : 'text-zinc-500 hover:text-white'}`}>
        ...
        <button onClick={() => signOut()} className="p-5 mt-4 text-zinc-600 font-black text-[10px] uppercase hover:text-red-500 border border-white/5 bg-black rounded-2xl flex justify-center gap-4 w-full transition-all"><LogOut size={16}/> DESCONECTAR</button>
      </aside>
      ...
      <main className="flex-1 p-12 overflow-y-auto bg-[#050505]">
        <header className="mb-16 flex justify-between items-end border-b border-white/5 pb-8">
          ...
          <div className="flex items-center gap-4 bg-[#0a0a0a] p-4 rounded-[2rem] border border-white/5">
             ...
             <img src={userSession?.user?.image || `https://ui-avatars.com/api/?name=${userSession?.user?.name}&background=EAB308&color=000`} className="w-12 h-12 rounded-[1.2rem] border-2 border-yellow-400/30 shadow-lg" alt="" />
          </div>
        </header>
             ...
             <form onSubmit={handleEnviar} className="bg-[#0a0a0a] border border-white/5 rounded-[3.5rem] p-12 shadow-2xl space-y-6">
                ...
                {form.tipo === 'VENDA' && (
                  <>
                    ...
                    <InputField label="DATA VENCIMENTO RESTANTE" type="date" value={form.dataVencimento} onChange={(v:any)=>setForm({...form, dataVencimento: v})} required={false} />
                  </>
                )}
                ...
                {form.tipo === 'SAQUE' && <InputField label="VALOR ENVIADO (R$)" type="number" value={form.valor} onChange={(v:any)=>setForm({...form, valor: v})} placeholder="Valor transferido ao membro" />}
        ...
        {activeTab === 'admin_zone' && (
           <div className="space-y-8 animate-in fade-in">
             <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="bg-red-500/5 border border-red-500/20 p-10 rounded-[3rem] shadow-lg">
                   <h3 className="text-3xl font-black italic text-white mb-4">FECHAMENTO MENSAL</h3>
                   <p className="text-zinc-500 mb-8 text-[10px] font-black uppercase tracking-widest">Arquiva o mês e salva os saldos. Use no dia 1º.</p>
                   <button onClick={virarMes} disabled={loading} className="w-full bg-red-600 text-white font-black py-5 rounded-2xl uppercase text-xs shadow-xl hover:bg-red-500 transition-all">VIRADA DE MÊS</button>
                </div>
                <div className="bg-purple-500/5 border border-purple-500/20 p-10 rounded-[3rem] shadow-lg">
                   <h3 className="text-3xl font-black italic text-white mb-4">AUDITORIA DE DADOS</h3>
                   <p className="text-zinc-500 mb-8 text-[10px] font-black uppercase tracking-widest">Desfaz o erro da virada e restaura Março.</p>
                   <button onClick={forcarAuditoria} disabled={loading} className="w-full bg-purple-600 text-white font-black py-5 rounded-2xl uppercase text-xs shadow-xl hover:bg-purple-500 transition-all">RESTAURAR MARÇO</button>
                </div>
             </div>
      ...
      {modalParcela && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/95 p-4 animate-in zoom-in-95">
           <div className="bg-[#0a0a0a] border border-zinc-800 p-12 rounded-[3.5rem] w-full max-w-md shadow-2xl relative">
              <button onClick={() => setModalParcela(null)} className="absolute top-8 right-8 text-zinc-600 hover:text-red-500 bg-black p-2 rounded-full border border-white/5 transition-all"><X size={24}/></button>
